...
const { userService, contactService ,NotificationEmailsService} = require("../services");
...
const contactus = catchAsync(async (req, res) => {
  ...
  const emails = await NotificationEmailsService.getEmail();
  await sendNotificationmail(emails,req.body);
  res.send(contact);
});
