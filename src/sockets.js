...
function createSocketsServer(server) {
  ...
  io.on("connection", async (socket) => {
    // console.log(socket);
    if (
      socket.handshake.headers.origin + "/" == config.clientUserUrl &&
      !liveVisitors.some((user) => user.socketId === socket.id)
    ) {
      const ip =
    ...
    socket.on("new-staff-add", (newUserId) => {
      if (newUserId && !activeStaff.some((user) => user.userId === newUserId)) {
        // if user is not added previously
        activeStaff.push({
          userId: newUserId,
          socketId: socket.id,
    ...
    socket.on("disconnect", (reason) => {
      ...
      liveVisitors = liveVisitors.filter((user) => user.socketId !== socket.id);
    ...
    socket.on("forceDisconnect", () => {
      ...
      socket.emit("get-staff", activeStaff);
    });
